...
// =================== TITLE (tactical op-briefing) ===================
function drawTitle(c) {
  // command-table backdrop
  const bg = c.createLinearGradient(0, 0, 0, VIEW_H);
  bg.addColorStop(0, '#04070a'); bg.addColorStop(0.6, '#060a0e'); bg.addColorStop(1, '#080d10');
  c.fillStyle = bg; c.fillRect(0, 0, VIEW_W, VIEW_H);
  const mcx = VIEW_W / 2, mcy = VIEW_H / 2 + 34;
  ...
  // surveyor grid
  c.strokeStyle = 'rgba(90,220,150,0.06)'; c.lineWidth = 1; c.beginPath();
  for (let x = 0.5; x <= VIEW_W; x += 32) { c.moveTo(x, 0); c.lineTo(x, VIEW_H); }
  for (let y = 0.5; y <= VIEW_H; y += 32) { c.moveTo(0, y); c.lineTo(VIEW_W, y); }
  c.stroke();
  // radar sweep over the map
  c.save(); c.globalCompositeOperation = 'lighter';
  const swA = G.rt * 0.7;
  for (let k = 0; k < 12; k++) {
    c.globalAlpha = 0.03 * (1 - k / 12); c.fillStyle = '#3aff9a';
    c.beginPath(); c.moveTo(mcx, mcy); c.arc(mcx, mcy, 300, swA - k * 0.05, swA - k * 0.05 + 0.05); c.closePath(); c.fill();
  }
  c.globalAlpha = 1; c.restore();
  // pulsing objective reticles on real POIs
  if (WORLD) {
    const R2 = Math.SQRT1_2, K = 350 / (WORLD.W * TILE);
    const mark = (wx, wy, col) => {
      const u = (wx - WORLD.W * TILE / 2) * K, v = (wy - WORLD.H * TILE / 2) * K;
      const x = mcx + (u - v) * R2, y = mcy + (u + v) * R2, r = 5 + Math.sin(G.rt * 2.4 + wx) * 1.6;
      c.strokeStyle = col; c.globalAlpha = 0.75;
      c.beginPath(); c.arc(x, y, r, 0, 7); c.stroke();
      c.fillStyle = col; c.fillRect(x - 0.5, y - r - 2, 1, 4); c.fillRect(x - 0.5, y + r - 2, 1, 4);
      c.fillRect(x - r - 2, y - 0.5, 4, 1); c.fillRect(x + r - 2, y - 0.5, 4, 1);
      c.globalAlpha = 1;
    };
    mark(WORLD.shops.bar.x, WORLD.shops.bar.y, '#ff2a6d');
    mark(WORLD.shops.guns.x, WORLD.shops.guns.y, '#f9f002');
    mark(WORLD.shops.cars.x, WORLD.shops.cars.y, '#00ff9f');
    mark(WORLD.skippySpot.x, WORLD.skippySpot.y, '#ff9f1c');
  }
  // dossier stamps
  c.globalAlpha = 0.5;
  drawText(c, 'OP: NIGHT CITY // GRID 2077-06', 10, 10, '#3aff9a', 1);
  drawText(c, 'SAT-LINK: MILITECH K-9 // LIVE', 10, 20, '#2a5a42', 1);
  drawTextR(c, 'CLASSIFIED — EYES ONLY', VIEW_W - 10, 10, '#ff5a5a', 1);
  c.globalAlpha = 1;
  const hz = c.createLinearGradient(0, VIEW_H - 120, 0, VIEW_H);
  hz.addColorStop(0, 'rgba(6,9,12,0)'); hz.addColorStop(1, 'rgba(5,8,10,0.72)');
  c.fillStyle = hz; c.fillRect(0, VIEW_H - 120, VIEW_W, 120);
  drawRain(c);

  // ---- stencil logo ----
  c.globalCompositeOperation = 'lighter'; c.globalAlpha = 0.4;
  c.drawImage(SPR.glowS('#3aff9a', 90), VIEW_W / 2 - 90, 18, 180, 90);
  c.globalAlpha = 1; c.globalCompositeOperation = 'source-over';
  const gx = Math.random() < 0.06 ? (Math.random() * 5 - 2.5) | 0 : 0;
  drawTextC(c, 'NIGHT CITY', VIEW_W / 2 - 3 + gx, 44, '#3aff9a', 5);
  drawTextC(c, 'NIGHT CITY', VIEW_W / 2 + 3 + gx, 44, '#ff2a3c', 5);
  drawTextC(c, 'NIGHT CITY', VIEW_W / 2 + gx, 44, '#eef6ee', 5);
  // stencil rule with end brackets
  c.fillStyle = 'rgba(90,255,150,0.35)'; c.fillRect(VIEW_W / 2 - 150, 86, 300, 1);
  c.fillStyle = '#7dff5a';
  for (const dx of [-153, 149]) { c.fillRect(VIEW_W / 2 + dx, 82, 1, 9); c.fillRect(VIEW_W / 2 + dx + (dx < 0 ? 1 : -3), 82, 3, 1); c.fillRect(VIEW_W / 2 + dx + (dx < 0 ? 1 : -3), 90, 3, 1); }
  drawTextC(c, 'T A C T I C A L   E D I T I O N', VIEW_W / 2, 92, '#7dff5a', 1);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (on) {
      ...
      g.addColorStop(0, 'rgba(58,255,154,0)'); g.addColorStop(0.5, 'rgba(58,255,154,0.14)'); g.addColorStop(1, 'rgba(58,255,154,0)');
      c.fillStyle = g; c.fillRect(VIEW_W / 2 - 150, y - 4, 300, 19);
      c.fillStyle = '#7dff5a';
      for (const dx of [-132, 124]) { c.fillRect(VIEW_W / 2 + dx, y - 4, 1, 19); c.fillRect(VIEW_W / 2 + dx + (dx < 0 ? 1 : -4), y - 4, 4, 1); c.fillRect(VIEW_W / 2 + dx + (dx < 0 ? 1 : -4), y + 14, 4, 1); }
      drawTextC(c, '▸ ' + items[i] + ' ◂', VIEW_W / 2, y, '#dfffe2', 2);
    } else {
...
function titleGender(c) {
  milEnsure();
  drawTextC(c, 'CHOOSE YOUR OPERATIVE', VIEW_W / 2, 128, '#7dff5a', 2);
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (hot && G.mouse.moved) G.uiS.sel = i;
    if (sel) { c.globalCompositeOperation = 'lighter'; c.globalAlpha = 0.3; c.drawImage(SPR.glowS('#3aff9a', 72), cx - 72, fy - 8, 144, 144); c.globalAlpha = 1; c.globalCompositeOperation = 'source-over'; }
...
function drawHUD(c) {
  const p = G.p;
  // ---- tactical vitals plate (bottom-left) ----
  const hpF = Math.max(0, Math.min(1, p.hp / p.maxhp));
  const hpCol = hpF < 0.35 ? '#ff2030' : hpF < 0.7 ? '#ffb02a' : '#4aff8a';
  c.fillStyle = 'rgba(5,8,12,0.78)'; c.fillRect(6, VIEW_H - 48, 132, 22);
  c.strokeStyle = 'rgba(90,255,150,0.3)'; c.strokeRect(6.5, VIEW_H - 47.5, 131, 21);
  c.fillStyle = '#4aff8a';
  c.fillRect(6, VIEW_H - 48, 6, 1); c.fillRect(6, VIEW_H - 48, 1, 5);
  c.fillRect(132, VIEW_H - 48, 6, 1); c.fillRect(137, VIEW_H - 48, 1, 5);
  c.fillRect(6, VIEW_H - 27, 6, 1); c.fillRect(6, VIEW_H - 31, 1, 5);
  c.fillRect(132, VIEW_H - 27, 6, 1); c.fillRect(137, VIEW_H - 31, 1, 5);
  drawText(c, 'VIT', 11, VIEW_H - 44, '#6a9a7a', 1);
  c.fillStyle = 'rgba(0,0,0,0.6)'; c.fillRect(30, VIEW_H - 45, 76, 6);
  c.fillStyle = hpCol; c.fillRect(30, VIEW_H - 45, 76 * hpF, 6);
  c.fillStyle = 'rgba(0,0,0,0.35)'; for (let t = 38; t < 106; t += 8) c.fillRect(t, VIEW_H - 45, 1, 6);
  drawTextR(c, String(Math.ceil(p.hp)), 134, VIEW_H - 44, '#e8ffe8', 1);
  drawText(c, 'LV' + G.lvl, 11, VIEW_H - 35, '#05d9e8', 1);
  if (p.armor > 0) drawText(c, 'ARM ' + p.armor, 44, VIEW_H - 35, '#8a93a6', 1);
  if (G.pHidden && !G.driving) drawText(c, 'CONCEALED', 84, VIEW_H - 35, '#00ff9f', 1);
  // XP ribbon (very bottom) + eddies
  ...
  // ---- hotbar: cyberware abilities + maxdoc as skill buttons ----
...
function drawMinimap(c) {
  // circular tactical radar, rotated 45° so its north matches the military camera
  const p = G.p, S = 64, mx = VIEW_W - S - 8, my = 8, cx = mx + S / 2, cy = my + S / 2, r = S / 2 + 2;
  ...
  for (let k = 0; k < 8; k++) {
    c.globalAlpha = 0.05 * (1 - k / 8); c.fillStyle = '#3aff9a';
    c.beginPath(); c.moveTo(cx, cy); c.arc(cx, cy, r, swA - k * 0.09, swA - k * 0.09 + 0.09); c.closePath(); c.fill();
  }
  c.globalAlpha = 1; c.globalCompositeOperation = 'source-over';
  const K = S * 1.5 / range, R2 = Math.SQRT1_2;
  const dot = (wx, wy, col, txt) => {
    const u = (wx / TILE - tx - range / 2) * K, v = (wy / TILE - ty - range / 2) * K;
    const dx = (u - v) * R2, dy = (u + v) * R2;
    if (dx * dx + dy * dy > (r - 4) * (r - 4)) return;
    if (txt) drawText(c, txt, cx + dx - 2, cy + dy - 2, col, 1);
    else { c.fillStyle = col; c.fillRect(cx + dx - 1, cy + dy - 1, 2, 2); }
  };
  ...
  dot(p.x, p.y, '#e8f6ff');
  c.restore();
  // rim + range ring + cardinal tick
  c.strokeStyle = 'rgba(90,255,150,0.55)'; c.beginPath(); c.arc(cx, cy, r, 0, 7); c.stroke();
  c.strokeStyle = 'rgba(90,255,150,0.14)'; c.beginPath(); c.arc(cx, cy, r * 0.55, 0, 7); c.stroke();
  c.fillStyle = '#7dff5a'; c.fillRect(cx - 0.5, cy - r - 3, 1, 3);
}
...
function drawCarThumb(c, cx, cy, sc, id, ang) {
  drawCarMil(c, cx, cy + 2 * sc, ang == null ? -0.6 : ang, CARD[id], sc * 1.05);
}
