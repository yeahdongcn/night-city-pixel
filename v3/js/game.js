...
function boot() {
  ...
  if (/autostart|demo/.test(q)) { startGame(false, /v=f/.test(q) ? 'f' : 'm'); forceWx(); }
    ...
    for (let i = 0; i < 20; i++) step(1 / 60); G.bannerO = null;
  } else if (/carspin/.test(q)) { // screenshot helper: driving the car (headlight beams on)
    startGame(false); G.cars.alvarado = 1; G.activeCar = 'alvarado'; G.summonCd = 0; summonCar();
    if (G.car) { G.car.x = G.p.x + 34; G.car.y = G.p.y - 14; G.car.a = -2.36; enterCar(); }   // facing "north"
...
// ===================================================================
// ======== TACTICAL RENDER OVERRIDES (v3 — Commandos fake-3D) =======
// The v1 sim is a flat world-pixel plane; these later declarations win and
// re-skin ONLY the rendering: military projection (a 45°-rotated plan with no
// foreshortening — the classic Commandos camera), a realistic pre-rendered
// ground with baked long shadows, real building facades, painterly actors.
// All systems (weapons, cars, cyberware, bounties, shops, weather, saves…)
// are unchanged and the save is shared with v1/v2.
// ===================================================================
...
function crateHidden(cr) {
  return WORLD.tileAt(cr.x, cr.y) >= 5 && milRoofAlphaAt((cr.x / TILE) | 0, (cr.y / TILE) | 0) > 0.55;
}
// how visible is an INDOOR actor at (x,y)? 0 = roof solid (hidden), 1 = roof gone.
// v1 drew indoor entities under the roof canvases; here we fade them in with the roof.
function milIndoorVis(x, y) {
  if (WORLD.tileAt(x, y) < 5) return 1;                    // outdoors
  const B = milBldAt(x, y);
  let a = milRoofAlphaAt((x / TILE) | 0, (y / TILE) | 0);
  if (B && B._fa != null && B._fa < a) a = B._fa;           // ghosted building reveals its interior too
  return Math.max(0, Math.min(1, 1 - a * 1.6 + 0.25));      // mostly-solid roof → invisible
}

// should this building ghost out because it covers V or an on-screen enemy?
// tight test: the actor's SPRITE must actually overlap the building's front
// silhouette (its wall-base line, sampled at the actor's screen x) — merely
// standing near a facade doesn't ghost the whole block.
function milOccludes(x0, y0, x1, y1, zh) {
  const nearD = x1 + y1;
  const pW = proj(x0, y1, 0), pE = proj(x1, y0, 0), pN = proj(x0, y0, 0), pS = proj(x1, y1, 0);
  for (let i = 0; i < G._occ.length; i++) {
    ...
    const base = a.sx <= pS.x
      ...
      : pS.y + (pE.y - pS.y) * ((a.sx - pS.x) / Math.max(1, pE.x - pS.x));  // along the east wall base
    if (a.sy - hSpr > base) continue;                      // sprite entirely below the wall base → in front, uncovered
    if (a.sy < pN.y - zh - 6) continue;                    // entirely above the roof line
    return true;
  }
  return false;
}

// per-shape plan silhouettes (shared with the mil car painter)
const CAR_SHAPE = {
...
function drawCarGrid(c) {
  ...
  milEnsure();
  const def = CARD[G._cargrid], cols = 6, rows = 2, n = 12;
  ...
  for (let k = 0; k < n; k++) {
    ...
    drawCarMil(c, cx, cy, ang, def, 1.4);
    drawTextC(c, (ang * 57.3 | 0) + 'DEG', cx, cy + 34, '#5a6372', 1);
  }
}
...
function render() {
  ...
  G._pscr = proj(p.x, p.y, 0); G._pdep = p.x + p.y;
  ...
  const wx0 = Math.min(a0.x, a1.x, a2.x, a3.x), wx1 = Math.max(a0.x, a1.x, a2.x, a3.x);
  const wy0 = Math.min(a0.y, a1.y, a2.y, a3.y), wy1 = Math.max(a0.y, a1.y, a2.y, a3.y);
  const minTX = clamp((wx0 / TILE - 1) | 0, 0, W - 1), maxTX = clamp((wx1 / TILE + 7) | 0, 0, W - 1);
  const minTY = clamp((wy0 / TILE - 1) | 0, 0, WORLD.H - 1), maxTY = clamp((wy1 / TILE + 7) | 0, 0, WORLD.H - 1);

  // ---- pre-rendered ground, rotate-blitted, plus live plan-space bits ----
  c.save();
  ...
  // puddle shimmer
  for (const pd of WORLD.puddles) {
    if (!isoVisible(pd.x, pd.y, 30)) continue;
    c.globalAlpha = 0.07 + 0.05 * Math.sin(G.rt * 2 + pd.x); c.fillStyle = pd.col;
    c.beginPath(); c.ellipse(pd.x, pd.y, pd.w / 2.4, pd.h / 2.4, 0, 0, 7); c.fill();
  }
  c.globalAlpha = 1;
  // Commandos view cones (same Kiroshi rule as v1 — only the look changed)
  if (G.cyber.kiroshi) for (const e of G.enemies) if (!e.dead && !e.alerted && isoVisible(e.x, e.y, 120)) milCone(c, e);
  c.restore();

  // ---- depth-sorted world: buildings, mountains, actors, props ----
  const D = [];
  for (const B of MILG.bld) {
    const zh = B.hgt * MIL_ZK;
    const pW2 = proj(B.x0, B.y1, 0), pE2 = proj(B.x1, B.y0, 0), pN2 = proj(B.x0, B.y0, 0), pS2 = proj(B.x1, B.y1, 0);
    if (pE2.x < -20 || pW2.x > VIEW_W + 20 || pS2.y < -20 || pN2.y - zh > VIEW_H + 20) continue;
    D.push({ d: B.x1 + B.y1, k: 'bld', B });
  }
  for (let ty = minTY; ty <= maxTY; ty++) for (let tx = minTX; tx <= maxTX; tx++) {
  ...
  // occluders to keep visible: V + every on-screen enemy (buildings in front of them ghost)
  G._occ = [{ sx: G._pscr.x, sy: G._pscr.y, d: G._pdep, big: G.driving }];
  for (const e of G.enemies) {
    if (e.dead) continue;
    const es = proj(e.x, e.y, 0);
    if (es.x > -20 && es.x < VIEW_W + 20 && es.y > -20 && es.y < VIEW_H + 40) G._occ.push({ sx: es.x, sy: es.y, d: e.x + e.y, big: e.big || e.psycho });
  }
  D.sort((u, v) => u.d - v.d);
  for (const it of D) drawMilThing(c, it, p);
  // name banners + holo boards after the tall stuff so they're never buried
  for (const sg of WORLD.signs) drawMilSign(c, sg);
